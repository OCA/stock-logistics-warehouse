...
class StockValuationLayerLotRevaluation(models.TransientModel):
    _name = "stock.valuation.layer.lot.revaluation"
    _description = "Wizard to revalue stock for a specific lot/serial number"
    ...
    def default_get(self, default_fields):
        ...
        if res.get("product_id"):
            ...
            if product.quantity_svl <= 0:
                raise UserError(
                    _(
                        "You cannot revalue a product with an empty "
                        "or negative stock."
                    )
                )
            if (
        ...
        if res.get("lot_id"):
            ...
            if lot.quantity_svl <= 0:
                raise UserError(
                    _("You cannot revalue a lot with an empty or negative stock.")
                )
        return res
    ...
    current_value_svl = fields.Float(
        string="Current Value",
        related="lot_id.value_svl",
    )
    current_quantity_svl = fields.Float(
        string="Current Quantity",
        related="lot_id.quantity_svl",
    )

    added_value = fields.Monetary(
        string="Added value",
        required=True,
    )
    ...
    def action_validate_revaluation(self):
        """Revaluate the stock for `self.lot_id` in `self.company_id`.
        ...
        - Distribute the `added_value` on the remaining_value of layers still in
          stock (with a remaining quantity)
        - If the Inventory Valuation of the product category is automated,
          create related account move.
        """
        self.ensure_one()
        ...
        if self.specific_ident_cost and self.lot_id:
            domain.append(("lot_id", "=", self.lot_id.id))

        remaining_svls = self.env["stock.valuation.layer"].search(domain)
        ...
        if self.specific_ident_cost and self.lot_id:
            revaluation_svl_vals["lot_id"] = self.lot_id.id
        ...
        for svl in remaining_svls:
            ...
            if (
                ...
                raise UserError(
                    _(
                        "The value of a stock valuation layer cannot "
                        "be negative. Landed cost could be used to "
                        "correct a specific transfer."
                    )
                )
        ...
        if product_id.categ_id.property_cost_method in ("average", "fifo"):
            product_id.with_context(disable_auto_svl=True).standard_price = (
                self.added_value + self.current_value_svl
            ) / self.current_quantity_svl
        ...
        account_move_line_name = _(
            "%(user)s changed stock valuation from  %(previous)s to "
            "%(new_value)s - %(product_lot)s",
            user=self.env.user.name,
            previous=previous_value_svl,
            new_value=previous_value_svl + self.added_value,
            product_lot=self.lot_id and self.lot_id.name or product_id.display_name,
        )
        move_vals = {
