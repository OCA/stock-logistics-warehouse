...
class IncomingCheckpointSelectionWizard(models.TransientModel):
    _name = "incoming.checkpoint.selection.wizard"
    _description = "Select data for incoming.checkpoint"

    purchase_ids = fields.Many2many(
        comodel_name="purchase.order",
        string="Purchases",
        help="Only approved, done or in exception orders",
    )
    ...
    def default_get(self, field_lists):
        res = super(IncomingCheckpointSelectionWizard, self).default_get(field_lists)
        old_obj = self.search(
