...
class TestVerticalLiftTrayType(VerticalLiftCase):
    ...
    def setUpClass(cls):
        ...
        cls.used_tray_type = cls.env.ref(
            'stock_vertical_lift.vertical_lift_tray_type_large_16x'
        )
        cls.unused_tray_type = cls.env.ref(
            'stock_vertical_lift.vertical_lift_tray_type_small_16x_3'
        )

    def test_tray_type(self):
        # any location created directly under the view is a shuttle
        tray_type = self.env['vertical.lift.tray.type'].create(
            {
                'name': 'Test Type',
                'code': '🐵',
                'usage': 'internal',
