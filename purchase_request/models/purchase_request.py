...
class PurchaseRequest(models.Model):
        ...
        for record in self:
            if record.requested_by:
                ...
                if employee.parent_id:
                    manager_user = self.env['res.users'].search([('employee_ids', '=', employee.parent_id.id)], limit=1)
                    # Mengecek apakah manajer adalah bagian dari grup "arkana_purchase_request.group_purchase_request_manager"
                    group_manager = self.env.ref('purchase_request.group_purchase_request_manager')
                    if group_manager in manager_user.groups_id:
                        record.assigned_to = manager_user
                    else:
    ...
    def _compute_is_approver(self):
        for record in self:
            record.is_approver = record.assigned_to == self.env.user
    
    @api.depends('state', 'is_approver')
    def _compute_show_approver_button(self):
        for record in self:
            record.show_approver_button = (record.state in ['draft','to_approve', 'approved',]) and record.is_approver
    ...
    date_expected = fields.Date(string="Expected Date", default=fields.Date.context_today, tracking=True)
    requested_by = fields.Many2one(
        "res.users", required=True, copy=False, tracking=True,
        default=_get_default_requested_by, index=True, readonly=True
    )
    assigned_to = fields.Many2one(
        "res.users", string="Approver", tracking=True, index=True,
        compute='_compute_assigned_to', store=True, readonly=True,
        compute_sudo=True
    )
    is_approver = fields.Boolean(compute='_compute_is_approver')
    show_approver_button = fields.Boolean(compute="_compute_show_approver_button")
