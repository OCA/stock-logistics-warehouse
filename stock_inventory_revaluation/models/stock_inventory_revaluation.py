...
class StockInventoryRevaluation(models.Model):
    ...
    new_value = fields.Float(
        string='Credit/Debit amount',
        help="Enter the amount you wish to credit or debit from the current "
             "inventory value of the item. Enter credit as a negative value. "
             "Relevant only if the selected revaluation type is Inventory "
             "Credit/Debit.",
        digits=dp.get_precision('Account'))
    ...
    def _set_validate_accounting_parameters(self):
        ...
        if not self.decrease_account_id or not self.increase_account_id:
            raise UserError(
                _("Please add an Increase Account and a Decrease Account"))
        ...
        if not acc.get('stock_valuation'):
            raise UserError(_("Please add a Stock Valuation Account in the "
                              "Product Category"))
        self.stock_valuation_account_id = acc['stock_valuation']
        self.journal_id = self.journal_id or acc.get('stock_journal')
        if not self.journal_id:
            raise UserError(_("Please specify an accounting journal"))
        ...
        for move in self:
            if not move.move_id or not move.product_id:
                ...
                if not move.product_id.tracking == 'none':
                    lot_names = move.move_id.mapped('move_line_ids.lot_name')
                    move.name = "%s: %s" % (move.name, ", ".join(lot_names))
