...
class ResPartner(models.Model):
    ...
    def _compute_product_supplierinfo_primary(self):
        ...
        # this used to return an {userid : [product_template]}
        # we need product_product
        partner2products = dict(self.env.cr.fetchall())
        for this in self:
            if this.id in partner2products.keys():
                p_products = self.env['product.product'].search([
                    ('product_tmpl_id', 'in', partner2products[this.id])
                ]).ids
                this.primary_product_ids = p_products
