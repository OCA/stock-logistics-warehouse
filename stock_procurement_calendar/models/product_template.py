    ...
    def _compute_procurement_attendance_ids(self):
        ...
        for template in self:
            attendances = attendance_obj.search(
                template._get_procurement_calendar_domain()
            )
            template.procurement_attendance_ids = attendances

    @api.multi
    def _get_procurement_calendar_domain(self):
        domain = expression.AND([
            [('product_ids', 'in', self.product_variant_ids.ids)],
    ...
    def _compute_procurement_calendar_count(self):
        ...
        for product in self:
            product.procurement_calendar_count = len(attendances.filtered(
                lambda a, p=product: p.product_variant_ids.ids in
                a.product_ids.ids or
