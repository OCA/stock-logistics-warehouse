...
class TestStockInventoryDestinationLocation(SavepointCase):
    ...
    def test_1(self):
        ...
        inventory = self.stock_inventory.create(
            {"name": "adjustment test", "filter": "product", "product_id": product.id}
        )
        inventory._onchange_filter()
        inventory.action_start()
        line = fields.first(
            inventory.line_ids.filtered(lambda l: l.product_id.id == product.id)
        )
        ...
        move = self.stock_move.search(
            [("product_id", "=", product.id), ("inventory_id", "=", inventory.id)],
            limit=1,
        )

        self.assertEqual(move.location_id, line.product_id.property_stock_inventory)
    ...
    def test_2(self):
        ...
        new_location = self.stock_location.create(
            {"name": "new location", "usage": "inventory"}
        )
