...
class StockInventoryLine(models.Model):
    ...
    def _generate_moves(self):
        ...
        unusual_lines = self.filtered(
            lambda l: l.virtual_location_id
            and l.virtual_location_id != l.product_id.property_stock_inventory
        )
        ...
        for line in unusual_lines:
            ...
            if (
                float_utils.float_compare(  # found more than expected
                    diff, 0.0, precision_rounding=line.product_uom_id.rounding
                )
                < 0
            ):
                vals = line._get_move_values(
                    abs(diff), line.virtual_location_id.id, line.location_id.id, False
                )
            else:
                vals = line._get_move_values(
                    abs(diff), line.location_id.id, line.virtual_location_id.id, True
                )
