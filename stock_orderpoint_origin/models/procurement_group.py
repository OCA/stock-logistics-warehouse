...
class ProcurementGroup(models.Model):
    ...
    def run(self, procurements, raise_user_error=True):
        ...
        for procurement in procurements:
            ...
            if source_docs:
                # stock.picking object doesn´t have field procurement_group_id so we check if the source document is
                # a picking to use the correct field (group_id)
                source_groups = [x.procurement_group_id if x._name != 'stock.picking' else x.group_id for x in source_docs]
