    ...
    def _check_routing_location_unique(self, routing_type):
        ...
        for picking_type in self:
            ...
            if other:
                raise exceptions.ValidationError(
                    _(
                        "Another routing operation picking type (%s) exists for"
                        " the same %s location."
                    )
                    % (other.display_name, message_fragment)
                )

    @api.constrains("src_routing_location_ids", "default_location_src_id")
    def _check_src_routing_location_unique(self):
        self._check_routing_location_unique("src")

    @api.constrains("dest_routing_location_ids", "default_location_dest_id")
