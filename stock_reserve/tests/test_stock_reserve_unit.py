...
class StockMove(TransactionCase):
    ...
    def test_virtual_stock(self):
        ...
        self.assertEqual(self.product_sorbet.virtual_available, 10, "Stock is not updated.")
        ...
        self.assertEqual(self.product_sorbet.product_tmpl_id.reservation_count, 6.5)
        ...
        # self.assertEqual(self.sorbet_orderpoint.procurement_ids[0].state,'exception','procurement must be in exception as there is no rule for procurement')
        # self.assertEqual(self.sorbet_orderpoint.procurement_ids[0].product_qty,math.ceil(15 - 3.5),'wrong product qty ordered')

        # I release the reservation
        self.reserv_sorbet1.release()
        # I check Virtual stock of Sorbet after update reservation
        self.assertEqual(self.product_sorbet.virtual_available,9.5,"Stock is not updated.")
        ...
        self.assertEqual(self.product_sorbet.virtual_available,10.0,"Stock is not updated.")        
        ...
        self.assertEqual(action_dict['res_model'],'stock.move',"action model is not 'stock.move'")
        self.assertEqual(action_dict['res_id'],move.id, "action res_id is not correct")
        self.assertEqual(action_dict['id'],self.env.ref('stock.stock_move_action').id,"action not correct")
        self.assertEqual(action_dict['views'][0][0],self.env.ref('stock.view_move_form').id,"action view not correct")
        ...
        self.assertEqual(action_dict['res_model'],'stock.reservation', "action model is not 'stock.move'")
        self.assertTrue(action_dict['domain'], "wrong domain")
        self.assertEqual(action_dict['domain'][0],('product_id', '=', self.product_sorbet.id), "action domain is not correct")
        self.assertEqual(action_dict['id'],self.env.ref('stock_reserve.action_stock_reservation_tree').id, "action not correct")
        self.assertEqual(action_dict['context']['search_default_draft'],1, "wrong context")
        self.assertEqual(action_dict['context']['search_default_reserved'], 1, "wrong context")
        ...
        self.assertEqual(action_dict['domain'][0][:2],('product_id', 'in'), "action domain is not correct")
