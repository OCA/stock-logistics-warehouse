...
class TestPotentialQty(SavepointCase):
    ...
    def setup_demo_data(self):
        ...
        self.tmpl = self.browse_ref("product.product_product_4_product_template")
        #  First variant
        self.var1 = self.browse_ref("product.product_product_4d")
        #  Second variant
    ...
    def test_potential_qty_no_bom_for_company(self):
        ...
        inventory = self.env["stock.inventory"].create(
            {
                ...
                "location_ids": [(6, 0, self.wh_ch.lot_stock_id.ids)],
                "state": "confirm",
                "product_ids": [(6, 0, [self.ref("mrp.product_product_wood_panel")])],
            }
        )
        self.env["stock.inventory.line"].create(
            {
        ...
        inventory = self.env["stock.inventory"].create(
            {
                ...
                "location_ids": [(6, 0, self.wh_ch.lot_stock_id.ids)],
                "product_ids": [
                    (6, 0, [self.ref("mrp.product_product_computer_desk_bolt")])
                ],
                "state": "confirm",
            }
        )
        ...
        self.assertPotentialQty(
            test_user_tmpl,
            0,
            "The stock is not available if chicago company ",
        )
    ...
    def test_potential_qty(self):
        ...
        inventory = self.env["stock.inventory"].create(
            {
                "name": "Receive Mouses",
                "location_ids": [(6, 0, self.wh_main.lot_stock_id.ids)],
                "state": "confirm",
                "product_ids": [(6, 0, [self.ref("mrp.product_product_wood_panel")])],
            }
        )
    ...
    def test_component_stock_choice(self):
        ...
        p1 = self.product_model.create({"name": "Test product with BOM"})
        bom_p1 = self.bom_model.create(
            {
        ...
        self.bom_line_model.create(
            {"bom_id": bom_p1.id, "product_id": prod.id, "product_qty": 1}
        )
        ...
        component = self.product_model.create({"name": "component", "type": "product"})
        ...
        self.bom_line_model.create(
            {"bom_id": imac_bom.id, "product_id": component.id, "product_qty": 1}
        )
        ...
        self.config.set_param("stock_available_mrp_based_on", "virtual_available")
        p1.refresh()
        self.assertEqual(5.0, p1.potential_qty)
