...
class StockCycleCount(models.Model):
    ...
    def _prepare_inventory_adjustment(self):
        for rec in self:
            if rec.cycle_count_rule_id.rule_type == "abc":
                inv_abc_groups = self.env["stock.abc"].search([])
                all_done_cycle = rec.search(
                    [
                        ("cycle_count_rule_id", "=", rec.cycle_count_rule_id.id),
                        ("state", "=", "done"),
                    ]
                )
                done_int_adj = []
                ...
                for group in inv_abc_groups:
                    ...
                    for product in group.product_ids:
                        ...
                        if product not in all_done_products and len(product_list) < y:
                            ...
                            elif rec.company_id.count_all_products:
                                product_list.append(product.id)
                        else:
                            continue
                    all_product_list.extend(product_list)
                abc_data = {
                    "name": "{}-{}".format(rec.location_id.name, rec.date_deadline),
                    "cycle_count_id": rec.id,
                    "location_ids": [(4, rec.location_id.id)],
