...
class ProcurementOrder(models.Model):
    ...
    def _prepare_purchase_order_line(self, po, supplier):
        ...
        seller = self.product_id._select_seller(
            ...
            date=po.date_order
            and fields.Date.to_string(fields.Date.from_string(po.date_order))
            or None,
            uom_id=self.product_id.uom_po_id,
        )
        res["product_purchase_uom_id"] = (
            seller.min_qty_uom_id.id or self.product_id.uom_po_id.id
        )
        if seller.packaging_id:
            ...
            if new_uom_id.id != res["product_uom"]:
                ...
                qty = self.product_uom._compute_quantity(self.product_qty, new_uom_id)
...
class Orderpoint(models.Model):
    ...
    def subtract_procurements_from_orderpoints(self):
        ...
        procs_by_orderpoint = dict.fromkeys(self.ids, self.env["procurement.order"])
