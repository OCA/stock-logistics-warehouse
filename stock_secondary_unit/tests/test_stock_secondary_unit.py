...
class TestProductSecondaryUnit(BaseCommon):
    ...
    def _check_uncounted_receipt_backorder(self, dependency_type, cancel=False):
        self.warehouse.reception_steps = "two_steps"
        product = self.product_template.product_variant_ids[0]
        secondary_unit = product.product_tmpl_id.secondary_uom_ids[0]
        secondary_unit.write({"dependency_type": dependency_type, "factor": 10})
        ...
        self.assertEqual(picking.move_line_ids.secondary_uom_qty, 0)
        action = picking.button_validate()
        wizard = Form(
            self.env[action["res_model"]].with_context(**action["context"])
        ).save()
        if cancel:
            ...
            self.assertFalse(
                self.StockPicking.search([("backorder_id", "=", picking.id)])
            )
            self.assertEqual(picking.move_ids.move_dest_ids.product_uom_qty, 10)
            self.assertEqual(picking.move_ids.move_dest_ids.secondary_uom_qty, 1)
            self.assertEqual(picking.move_line_ids.secondary_uom_qty, 0)
            return
        wizard.process()
        backorder = self.StockPicking.search([("backorder_id", "=", picking.id)])
        self.assertEqual(backorder.move_ids.product_uom_qty, 10)
        self.assertEqual(backorder.move_ids.secondary_uom_qty, 1)
        next_move = picking.move_ids.move_dest_ids
        self.assertEqual(next_move.product_uom_qty, 10)
        self.assertEqual(next_move.secondary_uom_qty, 1)
        self.assertEqual(picking.move_line_ids.secondary_uom_qty, 0)
        backorder.button_validate()
        self.assertEqual(next_move.product_uom_qty, 20)
        self.assertEqual(next_move.secondary_uom_qty, 2)
        self.assertEqual(backorder.move_line_ids.secondary_uom_qty, 0)

    def test_uncounted_receipt_backorder_secondary_priority(self):
        self._check_uncounted_receipt_backorder("secondary_priority")

    def test_uncounted_receipt_backorder_independent(self):
        self._check_uncounted_receipt_backorder("independent")

    def test_uncounted_receipt_cancel_backorder(self):
        self._check_uncounted_receipt_backorder("secondary_priority", cancel=True)
