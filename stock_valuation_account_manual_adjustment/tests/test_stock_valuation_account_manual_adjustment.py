...
class TestProductInventoryAccountReconcile(TransactionCase):
    ...
    def setUp(self):
        ...
        self.account_cogs = self._create_account(
            acc_type, name, code, self.company
        )
        ...
        code = "revaluation"
        acc_type = expense_type
        self.account_revaluation = self._create_account(
            acc_type, name, code, self.company
        )
        ...
        self.product_average_1 = self._create_product(
            "average", standard_price, False, list_price
        )
        # Add default quantity
        quantity = 10.00
        self._update_product_qty(
            self.product_average_1, self.location, quantity
        )
    ...
    def _create_product(
        self, cost_method, standard_price, template, list_price
    ):
    ...
    def test_reconcile_product(self):
        ...
        self.assertEquals(self.product_average_1.valuation_discrepancy,
                          -100.0)

        wiz = (
            self.env["stock.valuation.account.mass.adjust"]
            .with_context(
