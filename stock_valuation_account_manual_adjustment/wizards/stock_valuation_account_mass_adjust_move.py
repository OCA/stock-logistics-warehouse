...
class StockValuationAccountMassAdjustMove(models.TransientModel):
    ...
    def process(self):
        ...
        for move in moves:
            if (
                ...
                # we don't want to account for internal moves or dropshipments
                # nor for not finished moves
                continue
            account_move_lines = self.env["account.move.line"].search(
                [("stock_move_id", "=", move.id), ("reconciled", "=", True)]
            )
            if account_move_lines:
                # we don't want to account if the some journal entries are
                # reconciled
                continue
    ...
    def create_valuation_entries(self, move):
        ...
        try:
            ...
            raise UserError(
                _(
                    "The move lines are not in a consistent states: they do "
                    "not share the same origin or destination company."
                )
            )
