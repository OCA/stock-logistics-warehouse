            ...
            create or replace view stock_report_prodlots as (
                ...
                from (
                    select -max(sm.id) as id,
                        ...
                        sm.product_id,
                        sm.prodlot_id,
                        -sum(sm.product_qty /uo.factor) + coalesce(lot.retained_stock, 0)as qty
                    ...
                    where state = 'done'
                    group by sm.location_id, sm.product_id, sm.product_uom, sm.prodlot_id, lot.retained_stock
                    union all
                    select max(sm.id) as id,
                    ...
                    left join stock_production_lot lot
                        on (lot.id=sm.prodlot_id)
                    where state = 'done'
                    group by sm.location_dest_id, sm.product_id, sm.product_uom, sm.prodlot_id, lot.retained_stock
                ) as report
